...
impl DynamixelProtocolHandler {
    ...
    /// # Examples
    /// ```no_run
    /// use servocom::{DynamixelProtocolHandler, servo::dynamixel::xl320};
    /// use std::time::Duration;
    ///
    /// let mut serial_port = serialport::new("/dev/serial/by-id/usb-1a86_USB_Single_Serial_5B79034031-if00", 1_000_000)
    ///     .timeout(Duration::from_millis(10))
    ///     .open()
    ///     .expect("Failed to open port");
    ///
