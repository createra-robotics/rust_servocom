...
macro_rules! generate_servo {
    ($servo_name:ident, v1, $($rest:tt)*) => {
        $crate::__generate_servo_with!(
            $servo_name, $crate::DynamixelProtocolHandler, v1, $($rest)*
        );
    };
    ($servo_name:ident, v2, $($rest:tt)*) => {
        $crate::__generate_servo_with!(
            $servo_name, $crate::DynamixelProtocolHandler, v2, $($rest)*
        );
    };
    ($servo_name:ident, feetech, $($rest:tt)*) => {
        $crate::__generate_servo_with!(
            $servo_name, $crate::FeetechProtocolHandler, feetech, $($rest)*
        );
    };
}

#[macro_export]
macro_rules! __generate_servo_with {
    ($servo_name:ident, $handler:path, $protocol:ident,
     $(reg: ($reg_name:ident, $reg_access:ident, $reg_addr:expr, $reg_type:ty, $conv:ident),)+
        ...
        $(
            $crate::generate_reg_access!($servo_name, $handler, $reg_name, $reg_access, $reg_addr, $reg_type, $conv);
        )*
    };
}
...
macro_rules! generate_protocol_constructor {
    ...
    ($servo_name:ident, feetech) => {
        paste::paste! {
            impl [<$servo_name:camel Controller>] {
                /// Feetech servos only have one protocol version, so the handler
                /// is attached automatically — call [`with_serial_port`] next.
                pub fn new() -> Self {
                    Self {
                        dph: Some($crate::FeetechProtocolHandler::new()),
                        serial_port: None,
                    }
                }
            }
...
macro_rules! generate_reg_read {
    ($servo_name:ident, $handler:path, $reg_name:ident, $reg_addr:expr, $reg_type:ty, None) => {
        paste::paste! {
    ...
    ($servo_name:ident, $handler:path, $reg_name:ident, $reg_addr:expr, $reg_type:ty, $conv:ident) => {
        paste::paste! {
            #[doc = concat!("Read register *", stringify!($name), "* (addr: ", stringify!($addr), ", type: ", stringify!($reg_type), ")")]
